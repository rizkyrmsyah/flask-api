...
class SessionController(object):
    ...
    def login(self, request:dict):
        ...
        if user is None or not bcrypt.check_password_hash(user.password, request['password']):
            return jsonify(message="Kami tidak bisa menemukan akun yang cocok dengan data yang kamu masukkan. Harap periksa kembali dan coba lagi."), HTTPStatus.UNPROCESSABLE_ENTITY
    ...
    def check_password(self, request:dict):
        ...
        if user is None or not bcrypt.check_password_hash(user.password, request['password']):
            return jsonify(message="Kata sandi yang kamu masukkan salah"), HTTPStatus.UNPROCESSABLE_ENTITY
